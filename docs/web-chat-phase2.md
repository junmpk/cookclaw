...
- 共享核心先解析并保存 SearchRequest，再把已确认的菜/汤数量、人数和排除项转换成
  Graph Brief；确定性复杂度路由自动判断是否进入三 Agent，不要求用户了解架构术语；
...
3. app/demo/server.py：Web `/api/chat` 的 bridge 分支；Graph Run 直接持有
   canonical `thread_id`，`chat_links` 原子维护每个 thread 的当前 Run 指针。
...
> 今晚两个人吃，想用鸡肉和西兰花做两菜一汤，不要花生，整体清淡一点。
...
Graph。也可以使用左侧快捷按钮“复杂配餐演示”。

## 边界与取舍

- bridge 只接受共享状态中已经形成的显式菜单请求；缺少菜/汤数量时继续走普通聊天，
  不让 Graph 自行猜测数量。
- 路由不额外调用 LLM：菜汤组合、菜单槽位、人数与硬约束形成可记录的分数和原因；
  显式“三 Agent”关键词仅保留为开发调试覆盖。
- 局部修订只按槽位替换同类型候选，不接受模型或用户提供的候选外 recipe ID；
  没有可核验替代项时，Graph 会进入 blocked，不拿重复菜凑数。
- Graph 的执行仍是 Mock；设备动作不属于 Agent 工具权限。
- Graph Run、事件和 checkpoint 保存在本地 Demo SQLite；Run 直接记录所属
  `thread_id`，checkpoint 使用 `thread_id + run_id + version` 组合隔离。公开入口的
  对话状态仍使用 InMemoryConversationStore，重启后建议新建聊天线程。
...
- 用 SQLite 当前 Run 指针、持久化 checkpoint 标识和版本号实现“同一聊天线程继续确认”；
